  ...
  function tutPages() {
    return [
      { t: '게임의 목표', build: function () {
        ...
        box.appendChild(tutP('각자 **본체(HP 40)** 가 있고, **상대 본체의 HP를 0으로 만들면 승리**합니다. 인스턴스를 필드에 깔고, 앞으로 밀고 나가 상대 본체를 두들기는 것이 큰 그림입니다.'));
      ...
      { t: '보드와 진영', build: function () {
        ...
        box.appendChild(tutLi('새 인스턴스는 항상 **내 홈칸(빈 칸)** 에만 놓을 수 있어요.'));
        box.appendChild(tutLi('인스턴스는 **상하좌우로 1칸씩** 움직여 앞으로 전진합니다(대각선 X).'));
      ...
      { t: '한 턴에 할 수 있는 것', build: function () {
        ...
        box.appendChild(tutLi('**① 선언** — 손패의 인스턴스 카드를 홈칸에 놓기 (액션 1)'));
        box.appendChild(tutLi('**② 이동** — 내 인스턴스를 옆 빈 칸으로 1칸 옮기기 (액션 1)'));
        ...
        box.appendChild(tutLi('**기본 공격** — 인스턴스마다 턴에 1번, 공짜 (다음 장에서 자세히)'));
      ...
      { t: '전투 — 기본 공격', build: function () {
        var box = el('div', {});
        box.appendChild(tutP('모든 인스턴스는 자기 턴에 **「옆칸」(상하좌우 4칸)의 적** 하나를 **무료로, 턴당 1회** 때립니다. 대각선은 닿지 않아요.'));
        box.appendChild(tutMini({ '3,2': tutTile('내 인스턴스', '#fff', SKIN.own), '2,2': tutTile('⚔', '#fff', hexa(SKIN.enemy, .85)), '4,2': tutTile('⚔', '#fff', hexa(SKIN.enemy, .85)), '3,1': tutTile('⚔ 적본체', '#fff', hexa(SKIN.enemy, .85)), '3,3': tutTile('⚔', '#fff', hexa(SKIN.enemy, .85)) }, { home: false }));
        box.appendChild(el('div', { class: 'mono', style: { fontSize: '10px', color: SKIN.muted, textAlign: 'center', marginBottom: '8px' } }, ['빨강 ⚔ = 내 인스턴스가 때릴 수 있는 옆칸 4개']));
        box.appendChild(tutLi('피해는 **공격력(ATK)** 만큼. 대상 **체력(HP)** 이 0이 되면 파괴됩니다.'));
        box.appendChild(tutLi('받은 피해는 **턴을 넘겨도 누적**됩니다(자동 회복 없음).'));
        box.appendChild(tutLi('**반격은 없습니다** — 때린 쪽만 피해를 줍니다. 방어는 memory 인스턴스의 벽·피해감소로.'));
        box.appendChild(tutLi('카드가 말하는 **「적」 = 적 인스턴스 + 적 본체**. 그래서 **피해를 주는 능력·포인터**도 범위·직선이 닿으면 **본체를 직접** 노릴 수 있어요. (봉쇄·강제 이동·약화 같은 조작은 인스턴스에만 적용 — 「적 인스턴스」/「적 본체」로 콕 집어 적힌 경우는 그 대상만.)'));
        return box;
      } },
      { t: '클래스 상성', build: function () {
        var box = el('div', {});
        box.appendChild(tutP('인스턴스는 세 클래스로 나뉘고 **가위바위보** 관계입니다:'));
        ...
        box.appendChild(tutP('카드의 능력 키워드: **When**(자동) · **If**(원하면 발동) · **Switch**(변신) · **While**(상시) · **For**(내 턴마다 수동, 무료).'));
        return box;
      } },
      ...
      { t: '능력 키워드 (함수)', tier: '심화', build: function () {
        ...
        box.appendChild(tutDefList([
          ...
          ['Switch', '**게임당 1회**, 원할 때 인스턴스를 **두 형태(폼) 중 하나로 영구 변신**. 공/체·능력이 선택한 폼에 맞게 바뀝니다. 변신 대상 폼은 도감·덱빌더에서 함께 볼 수 있어요.'],
        ...
        box.appendChild(tutP('필드 인스턴스에 커서를 올리면 **함수 범위**(🟡)가 보드에 표시됩니다. For 능력은 인스턴스를 클릭하면 나오는 **⚡ 발동**, If/Switch 능력은 **◆/⇄ 선택** 버튼으로 씁니다.'));
        return box;
      } },
      { t: '특수 상태 · 오라', tier: '심화', build: function () {
        var box = el('div', {});
        box.appendChild(tutP('전투 중 인스턴스·본체에 붙는 **주요 상태**입니다.'));
        box.appendChild(tutDefList([
        ...
        box.appendChild(tutP('적 인스턴스에 커서를 올려 어떤 오라·상태를 가졌는지 확인하고, **오라 인스턴스부터 걷어내는 것**이 공략의 기본입니다.'));
      ...
      { t: '런타임 환경 (RUNTIME ENV)', tier: '심화', build: function () {
        ...
        var rows = [
          ...
          ['⚡', '오버클럭', '#c8951b', '모든 인스턴스 공격력 **+1** (양측·즉시).'],
          ['🧊', '스로틀링', '#3f7bd6', '모든 인스턴스 공격력 **−1** (최소 0·즉시).'],
          ['🩸', '메모리 누수', '#c23c70', '**8턴부터** 매 턴 양쪽 본체 HP **−1** — 유닛은 무피해, 장기전을 끝내는 시계입니다.'],
