  ...
  function tutPages() {
    return [
      ...
      { t: '날씨 (RUNTIME WEATHER)', tier: '심화', build: function () {
        var box = el('div', {});
        box.appendChild(tutP('**게임을 시작할 때마다 「날씨」 1종이 무작위로 지정**되어, 그 판 내내 필드 전체에 영향을 줍니다. 시작 시 어떤 날씨인지 **연출로 공개**되고, 대국 화면 상단(타이틀바)과 컨트롤 바의 **배지**로 언제든 확인할 수 있어요. (온라인 대전은 양쪽에 **같은 날씨**가 적용됩니다.)'));
        var rows = [
