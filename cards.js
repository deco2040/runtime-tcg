...
(function () {
  window.RT_DEFINE_CARDS = function (kit) {
    ...
    // ============================================================ THREAD — objects
    def({ id: 'Fiber', cls: 'thread', kind: 'object', atk: 6, hp: 3, text: 'For(2) 「옆칸」 적 1명에게 공격력만큼 피해',
    ...
    def({ id: 'Worker', cls: 'thread', kind: 'object', atk: 4, hp: 3, text: 'For(3) 「1칸이내」 적 전부에게 3 피해',
      abilities: [{ kw: 'For', forCount: 3, trigger: 'onTurnStart', fn: function (G, u) { G.around(u).filter(function (x) { return x.owner !== u.owner && x.type === 'object'; }).forEach(function (x) { G.deal(x, 3, { attacker: u }); }); } }] });
    ...
    def({ id: 'Interrupt', cls: 'thread', kind: 'object', atk: 6, hp: 3, text: 'When 「옆칸」에 적 진입 시 공격력만큼 피해 · 「옆칸」 다른 아군 thread 1장당 +2',
    ...
    def({ id: 'Exec', cls: 'thread', kind: 'object', atk: 6, hp: 4, text: 'For(2) 「1칸이내」 적 전부에게 공격력만큼 피해',
      abilities: [{ kw: 'For', forCount: 2, trigger: 'onTurnStart', fn: function (G, u) { G.around(u).filter(function (x) { return x.owner !== u.owner && x.type === 'object'; }).forEach(function (x) { G.deal(x, G.effAtk(u), { attacker: u }); }); } }] });
    ...
    def({ id: 'Livelock', cls: 'thread', kind: 'object', atk: 7, hp: 4, mustAttack: true, text: 'While 「옆칸」에 공격 가능한 적이 있으면 반드시 1명을 기본 공격', abilities: [] });
    ...
    def({ id: 'overclock()', cls: 'thread', kind: 'pointer', need: 'allyThread', text: '내 thread 1장 공격력 2배(현재치만큼 증가)', cast: function (G, p, tk) { var u = G.board[tk]; if (u && u.owner === p && cardCls(u) === 'thread') G.buffAtk(u, G.effAtk(u)); } });
    def({ id: 'crash()', cls: 'thread', kind: 'pointer', need: 'allyThread', text: '내 thread 1장과 그 thread의 「옆칸」 적 1명이 서로에게 각자 공격력만큼 동시 피해',
    ...
    def({ id: 'strike()', cls: 'thread', kind: 'pointer', need: 'enemy', text: '받은 피해가 있는 적 1명에게 8 피해',
    ...
    def({ id: 'burst()', cls: 'thread', kind: 'pointer', need: 'enemy', text: '적 1명에게 (대상의 「옆칸」에 있는 내 thread 수 ×3) 피해', cast: function (G, p, tk) { var u = G.board[tk]; if (!u) return; var n = G.adj(u).filter(function (x) { return x.owner === p && cardCls(x) === 'thread'; }).length; G.deal(u, n * 3, { attacker: { owner: p } }); } });
    ...
    def({ id: 'rush()', cls: 'thread', kind: 'pointer', need: 'allyThread', text: '내 thread 1장 앞으로 「1칸이동」 + 공격력 +1',
    ...
    def({ id: 'reap()', cls: 'thread', kind: 'pointer', need: 'allyThread', text: '내 thread 1장 파괴, 적 1명에게 파괴한 thread의 공격력만큼 피해',
      castValid: function (G, p, tk) { var u = G.board[tk]; return !!u && u.owner === p && G.enemyObjects(p).length > 0; },
      cast: function (G, p, tk, o) { var u = G.board[tk]; if (!u) return; var atk = G.effAtk(u); var e = (o && o.second && G.board[o.second]) ? G.board[o.second] : strongestEnemy(G, p); G.destroy(u, { attacker: { owner: p } }); if (e && G.board[unitKey(G, e)]) G.deal(e, atk, { attacker: { owner: p } }); } });
    def({ id: 'jolt()', cls: 'thread', kind: 'pointer', need: 'enemy', text: '적 1명 3 피해, 대상의 「옆칸」에 내 thread 있으면 6 피해', cast: function (G, p, tk) { var u = G.board[tk]; if (!u) return; var boost = G.adj(u).some(function (x) { return x.owner === p && cardCls(x) === 'thread'; }); G.deal(u, boost ? 6 : 3, { attacker: { owner: p } }); } });
    ...
    // ============================================================ MEMORY — objects
    def({ id: 'Cache', cls: 'memory', kind: 'object', atk: 0, hp: 12, text: 'While 「옆칸」 적 이동 불가', abilities: [] });
    ...
    def({ id: 'Semaphore', cls: 'memory', kind: 'object', atk: 0, hp: 8, text: 'For(2) 적 1명 1턴 봉쇄',
    ...
    def({ id: 'Sentinel', cls: 'memory', kind: 'object', atk: 2, hp: 9, text: 'When 「옆칸」에 적 진입 시 그 적 1턴 봉쇄 + 2 피해',
    ...
    def({ id: 'Const', cls: 'memory', kind: 'object', atk: 2, hp: 10, text: 'While 자기 이동 불가 · 「옆칸」 적 봉쇄', abilities: [] });
    ...
    def({ id: 'Register', cls: 'memory', kind: 'object', atk: 3, hp: 7, text: 'For(1) 적 1명 공격력 0 (1턴)',
    ...
    def({ id: 'Trap', cls: 'memory', kind: 'object', atk: 1, hp: 11, text: 'Once 「옆칸」에 적 첫 진입 시 그 적에게 7 피해(게임 중 1회)',
    ...
    def({ id: 'Latch', cls: 'memory', kind: 'object', atk: 1, hp: 8, text: 'When 내 턴 종료 시 「옆칸」 적 2명 이상이면 전부 2 피해',
      abilities: [{ kw: 'When', trigger: 'onTurnEnd', fn: function (G, u) { var e = G.adj(u).filter(function (x) { return x.owner !== u.owner && x.type === 'object'; }); if (e.length >= 2) e.forEach(function (x) { G.deal(x, 2, { attacker: u }); }); } }] });
    ...
    def({ id: 'Collector', cls: 'memory', kind: 'object', atk: 2, hp: 9, text: 'For(1) 봉쇄된 적 1명에게 5 피해',
    ...
    // ============================================================ MEMORY — pointers
    def({ id: 'free()', cls: 'memory', kind: 'pointer', need: 'enemy', text: '내 본체 「2칸이내」 적 1명 6 피해', cast: function (G, p, tk) { var u = G.board[tk]; if (u) G.deal(u, 6, { attacker: { owner: p } }); } });
    def({ id: 'lock()', cls: 'memory', kind: 'pointer', need: 'enemy', text: '적 1명 2턴 봉쇄', cast: function (G, p, tk) { var u = G.board[tk]; if (u) G.bind(u, 2); } });
    def({ id: 'restore()', cls: 'memory', kind: 'pointer', need: 'allyOrBody', text: '아군 1장 또는 내 본체 6 회복', cast: function (G, p, tk) { var u = tk ? G.board[tk] : (woundedAlly(G, p) || G.body(p)); if (u) G.healInst(u, 6); } });
    ...
    def({ id: 'purge()', cls: 'memory', kind: 'pointer', need: 'enemy', text: '적 1명 공격력 0 (2턴)', cast: function (G, p, tk) { var u = G.board[tk]; if (u) G.setAtkZeroTurns(u, 2); } });
    ...
    def({ id: 'freeze()', cls: 'memory', kind: 'pointer', need: 'ally', text: '내 인스턴스 1장의 「2칸이내」 적 전부 1턴 봉쇄',
    ...
    def({ id: 'bind()', cls: 'memory', kind: 'pointer', need: 'none', castCondition: { type: 'turnCount', n: 4 }, text: '조건 내 턴 4회+ · 적 전부 1턴 봉쇄', cast: function (G, p) { G.enemyObjects(p).forEach(function (x) { G.bind(x, 1); }); } });
    ...
    def({ id: 'Raycast', cls: 'process', kind: 'object', atk: 6, hp: 5, text: 'For(1) 네 대각선 방향 2칸 내 적 1명에게 공격력만큼 피해',
    ...
    def({ id: 'Jump', cls: 'process', kind: 'object', atk: 5, hp: 5, text: 'For(2) 「3칸이내」 빈 칸으로 이동(경로의 벽·유닛 무시)',
    ...
    def({ id: 'Exploit', cls: 'process', kind: 'object', atk: 4, hp: 5, text: 'When 선언 시 적 1명 공격력 -3',
      abilities: [{ kw: 'When', trigger: 'onSummon', fn: function (G, u) { var t = strongestEnemy(G, u.owner); if (t) G.buffAtk(t, -3); } }] });
    def({ id: 'Reroute', cls: 'process', kind: 'object', atk: 3, hp: 6, require: { type: 'turnCount', n: 2 }, text: 'require 내 턴 2회+ 진행 · For(1) 적 1명을 그 「옆칸」 빈 칸으로 강제 이동',
    ...
    def({ id: 'Profiler', cls: 'process', kind: 'object', atk: 2, hp: 6, text: 'While 「1칸이내」 적 1명당 공격력 +1', abilities: [] });
    def({ id: 'Probe', cls: 'process', kind: 'object', atk: 4, hp: 5, text: 'For(1) 「3칸이내」 가장 먼 적 1명에게 공격력만큼 피해(동거리 시 내가 선택)',
    ...
    def({ id: 'Callback', cls: 'process', kind: 'object', atk: 3, hp: 5, text: 'When 파괴 시 「1칸이내」 적 1명 5 피해',
      abilities: [{ kw: 'When', trigger: 'onDeath', fn: function (G, u, ctx) { var t = G.objects().filter(function (x) { return x.owner !== u.owner && cheb(unitKey(G, x), ctx.atKey) <= 1; }).sort(function (a, b) { return G.curHp(a) - G.curHp(b); })[0]; if (t) G.deal(t, 5, { attacker: u }); } }] });
    ...
    def({ id: 'Honeypot', cls: 'process', kind: 'object', atk: 2, hp: 6, text: 'For(1) 「3칸이내」 적 1명을 자기 「옆칸」 빈 칸으로 끌어당김 + 2 피해',
    ...
    def({ id: 'Offset', cls: 'process', kind: 'object', atk: 5, hp: 4, text: 'For(1) 「나이트」 도약 위치(체스 나이트 모양)의 적 1명에게 공격력만큼 피해',
    ...
    def({ id: 'Dispatch', cls: 'process', kind: 'object', atk: 3, hp: 4, text: 'For(2) 내 인스턴스 1장을 「나이트」 도약 위치(체스 나이트 모양) 빈 칸으로 재배치(강제 이동 아님)',
    ...
    def({ id: 'Fault', cls: 'process', kind: 'object', atk: 3, hp: 5, text: 'While 적이 봉쇄될 때마다 그 적에게 1 피해', abilities: [] });

    // ============================================================ PROCESS — pointers
    def({ id: 'memcpy()', cls: 'process', kind: 'pointer', need: 'enemy', castCondition: { type: 'turnCount', n: 3 }, text: '조건 내 턴 3회+ · 적 1명을 그 「옆칸」 빈 칸으로 강제 이동',
    ...
    def({ id: 'push()', cls: 'process', kind: 'pointer', need: 'enemy', text: '적 1명 적 진영 쪽으로 「1칸이동」',
      castValid: function (G, p, tk) { var u = G.board[tk]; if (!u || u.owner === p) return false; var q = P(tk), nr = q[1] + fwd(p); return inB(q[0], nr) && !G.board[K(q[0], nr)]; },
      cast: function (G, p, tk) { var u = G.board[tk]; if (u) G.pushAway(u, p); } });
    def({ id: 'chain()', cls: 'process', kind: 'pointer', need: 'enemy', text: '적 1명과 그 뒤 같은 열 첫 적(적 진영 방향)에게 각 4 피해', cast: function (G, p, tk) { var u = G.board[tk]; if (!u) return; var back = G.chainBackEnemy(tk, p); G.deal(u, 4, { attacker: { owner: p } }); if (back && G.board[unitKey(G, back)]) G.deal(back, 4, { attacker: { owner: p } }); } });
    ...
    def({ id: 'trace()', cls: 'process', kind: 'pointer', need: 'allyProcess', text: '내 process 1장을 적 1명의 「1칸이내」 빈 칸으로 이동(경로의 벽·유닛 무시) + 그 적에게 2 피해',
    ...
    def({ id: 'jitter()', cls: 'process', kind: 'pointer', need: 'none', castCondition: { type: 'turnCount', n: 4 }, text: '조건 내 턴 4회+ · 적 전부 각자 「옆칸」 빈 칸으로 무작위 강제 이동(빈 칸 없으면 유지)',
    ...
    def({ id: 'splice()', cls: 'process', kind: 'pointer', need: 'enemy', castCondition: { type: 'turnCount', n: 3 }, text: '조건 내 턴 3회+ · 내 인스턴스 1장과 적 1명 위치 교환',
    ...
    def({ id: 'glitch()', cls: 'process', kind: 'pointer', need: 'enemy', text: '적 1명 4 피해 후 적 진영 쪽으로 「1칸이동」', cast: function (G, p, tk) { var u = G.board[tk]; if (!u) return; G.deal(u, 4, { attacker: { owner: p } }); if (G.board[tk]) G.pushAway(u, p); } });
    ...
    def({ id: 'hop()', cls: 'process', kind: 'pointer', need: 'allyProcess', text: '내 process 1장 「1칸이동」, 이동 후 「옆칸」 적 있으면 그중 1명 2 피해',
      castValid: function (G, p, tk) { var u = G.board[tk]; return !!u && u.owner === p && G.moveCells(u).length > 0; },
      cast: function (G, p, tk, o) { var u = G.board[tk]; if (!u) return; var dest = (o && o.dest && G.moveCells(u).indexOf(o.dest) >= 0) ? o.dest : (forwardDest(G, u) || G.moveCells(u)[0]); if (dest) G.move(u, dest, true); var t = G.adj(u).filter(function (x) { return x.owner !== p && x.type === 'object'; })[0]; if (t) G.deal(t, 2, { attacker: u }); } });
    ...
    def({ id: 'Bit', cls: 'generic', kind: 'object', atk: 1, hp: 2, text: 'For(3) 「옆칸」 적 1명에게 공격력만큼 피해', abilities: [{ kw: 'For', forCount: 3, trigger: 'onTurnStart', fn: function (G, u, ch) { var t = pickAdjEnemy(G, u, ch); if (t) G.deal(t, G.effAtk(u), { attacker: u }); } }] });
    ...
    def({ id: 'Coerce', cls: 'generic', kind: 'object', atk: 3, hp: 3, text: 'When 선언 시 적 1명 공격력 -2', abilities: [{ kw: 'When', trigger: 'onSummon', fn: function (G, u) { var t = strongestEnemy(G, u.owner); if (t) G.buffAtk(t, -2); } }] });
    ...
    def({ id: 'Merge', cls: 'generic', kind: 'object', atk: 4, hp: 4, text: 'When 파괴 시 「옆칸」 적 1명에게 4 피해', abilities: [{ kw: 'When', trigger: 'onDeath', fn: function (G, u, ctx) { var p = P(ctx.atKey); var t = ortho(p[0], p[1]).map(function (k) { return G.board[k]; }).filter(function (x) { return x && x.owner !== u.owner; }).sort(function (a, b) { return G.curHp(a) - G.curHp(b); })[0]; if (t) G.deal(t, 4, { attacker: u }); } }] });
    ...
    def({ id: 'Debug', cls: 'generic', kind: 'object', atk: 2, hp: 4, text: 'While 「옆칸」 적이 받는 피해 +1', abilities: [] });
    ...
    def({ id: 'flush()', cls: 'generic', kind: 'pointer', need: 'ally', text: '내 인스턴스 1장의 「옆칸」 적 전부 3 피해',
      castValid: function (G, p, tk) { var u = G.board[tk]; return !!u && u.owner === p && G.adj(u).some(function (x) { return x.owner !== p && x.type === 'object'; }); },
      cast: function (G, p, tk) { var u = tk ? G.board[tk] : G.allyObjects(p).filter(function (a) { return G.adj(a).some(function (x) { return x.owner !== p && x.type === 'object'; }); })[0]; if (!u) return; G.adj(u).forEach(function (x) { if (x.owner !== p && x.type === 'object') G.deal(x, 3, { attacker: { owner: p } }); }); } });
    ...
    def({ id: 'throw()', cls: 'generic', kind: 'pointer', need: 'enemy', text: '적 1명 4 피해, 대상의 「옆칸」 적 전부 2 피해', cast: function (G, p, tk) { var u = G.board[tk]; if (!u) return; var q = P(tk); G.deal(u, 4, { attacker: { owner: p } }); ortho(q[0], q[1]).map(function (k) { return G.board[k]; }).filter(function (x) { return x && x.owner !== p && x.type === 'object'; }).forEach(function (x) { G.deal(x, 2, { attacker: { owner: p } }); }); } });
    def({ id: 'catch()', cls: 'generic', kind: 'pointer', need: 'enemy', text: '적 1명 4 피해, 파괴 시 카드 1장 뽑기', cast: function (G, p, tk) { var u = G.board[tk]; if (!u) return; G.deal(u, 4, { attacker: { owner: p } }); if (!G.board[tk]) G.draw(p, 1); } });
    ...
    def({ id: 'clear()', cls: 'generic', kind: 'pointer', need: 'enemy', text: '적 1명의 공격력 강화 효과 전부 해제(기본치로)', cast: function (G, p, tk) { var u = G.board[tk]; if (u && u.atkMod > 0) u.atkMod = 0; } });
    ...
    def({ id: 'exit()', cls: 'generic', kind: 'pointer', need: 'none', castCondition: { type: 'selfBodyHP', cmp: '<=', n: 18 }, text: '조건 내 본체 HP 18 이하 · 모든 적 3 피해', cast: function (G, p) { G.enemyObjects(p).forEach(function (x) { G.deal(x, 3, { attacker: { owner: p } }); }); } });
    ...
    def({ id: 'Switch', cls: 'generic', kind: 'object', atk: 3, hp: 4, text: 'If 선택 발동 · [공격] 적 1명 3 피해 / [보호] 아군 1장 3 회복',
    ...
    def({ id: 'Branch', cls: 'process', kind: 'object', atk: 4, hp: 5, text: 'If 선택 발동 · [도약] 자기 「1칸이동」 후 「옆칸」 적 1명 2 피해 / [교란] 적 1명 적 진영 쪽으로 「1칸이동」',
      abilities: [{ kw: 'If', forCount: 1, trigger: 'onActive', options: [{ label: '도약' }, { label: '교란' }],
        ...
        fn: function (G, u, ch) { if ((ch.opt || 0) === 0) { var dest = forwardDest(G, u) || G.moveCells(u)[0]; if (dest) G.move(u, dest, true); var t = G.adj(u).filter(function (x) { return x.owner !== u.owner && x.type === 'object'; })[0]; if (t) G.deal(t, 2, { attacker: u }); } else { var e = ch.target ? G.board[ch.target] : bestEnemyObj(G, u.owner); if (e) G.pushAway(e, u.owner); } },
        aiOpt: function (G, u) { return 0; } }] });
    def({ id: 'Guard', cls: 'memory', kind: 'object', atk: 1, hp: 9, text: 'If 선택 발동 · [수호] 내 본체 3 회복 / [응징] 「옆칸」 적 1명 3 피해',
    ...
    def({ id: 'Singleton', cls: 'memory', kind: 'object', atk: 6, hp: 20, deckLimit: 1, require: { type: 'or', a: { type: 'turnCount', n: 5 }, b: { type: 'classOnBoard', cls: 'memory', n: 4 } }, text: '덱당 1 · require 내 턴 5회+ 또는 memory 4개+ 필드에 존재 · Once 선언 시 적 전부 1턴 봉쇄 · While 본체 받는 피해 -2',
    ...
    def({ id: 'Overlord', cls: 'generic', kind: 'object', atk: 6, hp: 6, deckLimit: 1, require: { type: 'and', a: { type: 'turnCount', n: 4 }, b: { type: 'selfBodyHP', cmp: '>=', n: 28 } }, text: '덱당 1 · require 내 턴 4회+ · 내 본체 HP 28+ · Once 선언 시 적 본체 6 피해 · For(2) 적 1명에게 공격력만큼 피해',
      abilities: [
        { kw: 'Once', trigger: 'onSummon', fn: function (G, u) { G.deal(G.enemyBody(u.owner), 6, { attacker: u }); } },
        { kw: 'For', forCount: 2, trigger: 'onTurnStart', fn: function (G, u, ch) { var t = ch.target ? G.board[ch.target] : bestEnemyObj(G, u.owner); if (t) G.deal(t, G.effAtk(u), { attacker: u }); } }
      ] });
