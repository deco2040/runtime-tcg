...
(function () {
  ...
  var RT_TIPS = [
    ...
    '주석과 코드가 서로 다른 말을 한다.',
    '"임시" 코드가 3년째 돌아간다.',
    '버그 99개를 고쳤다 — 이제 127개다.',
    '어려운 건 둘: 캐시 무효화, 이름 짓기, 그리고 off-by-one.',
    '재귀를 이해하려면 먼저 재귀를 이해하라.',
    '버그가 아니라 사양입니다.',
    '전역 변수는 미래의 나에게 보내는 저주다.',
    'null 은 10억 달러짜리 실수였다.',
    '엣지 케이스는 사용자가 먼저 찾는다.',
    '스택오버플로가 없으면 코드도 없다.',
    '주석 처리한 코드는 언젠가 돌아온다.',
    '마지막으로 딱 한 줄만 더 고치자.',
    '이 게임의 AI도 가끔 세그폴트를 꿈꾼다.',
    '런타임 환경은 고를 수 없다 — 주어진 날씨에 덱을 맞춰라.',
    '가끔은 아무 일도 일어나지 않는다. 그것도 하나의 환경이다.',
    'undefined 는 함수가 아닙니다.',
    '작동하는 코드는 건드리지 마라.',
    '캐시를 비웠더니 문제가 사라졌다. 이유는 묻지 마라.',
    '문서? 코드가 곧 문서다.'
  ];
  ...
  //   onscroll 이벤트 수·render 진입 수·제스처 플래그·COMPACT/터치 여부. 어느 레이아웃이 뜨는지까지 한눈에 확인.
  var _hudOn = false;
  function dbgHud() {
    if (_hudOn || !/[?&]dbg=1/.test(location.search || '') || !document.body) return;
    _hudOn = true;
    var d = el('div', { id: 'dbghud', style: { position: 'fixed', left: '3px', top: '3px', zIndex: 999999, background: 'rgba(0,0,0,.85)', color: '#3f6', fontFamily: 'monospace', fontSize: '11px', lineHeight: '1.35', padding: '4px 6px', whiteSpace: 'pre', pointerEvents: 'none', border: '1px solid #3f6' } }, ['dbg…']);
    document.body.appendChild(d);
    (function tick() {
      var h = document.getElementById('handrow');
      var n = (G && G.players && G.players[HUMAN]) ? G.players[HUMAN].hand.length : -1;
      d.textContent = 'COMPACT=' + (COMPACT ? 1 : 0) + ' coarse=' + (isTouchDevice() ? 1 : 0) + ' w=' + (window.innerWidth || 0)
  ...
  function renderMulliganDesktop(wrap) {
    ...
    var main = el('div', { style: { display: 'flex', gap: '8px', padding: 'clamp(2px,0.3vw,4px)', alignItems: 'stretch', flexWrap: 'wrap', width: '100%', maxWidth: '1400px', margin: '0 auto' } });
  ...
  function deskBoardMaxW() {
    ...
    var boardH = Math.max(150, Math.min(900, vh - chrome + _vfit)); // 세로가 남으면 최대 900px까지 보드로 채움(_vfit=실측보정)
    return Math.round(Math.min(1120, boardH * 1.25)); // 5:4 → 폭 = 높이 × 1.25 (폭 상한 1120, 큰 모니터 대응)
  }
  ...
  function fitBoardVertical() {
    ...
    var delta = slack - 2;                                     // 목표: 2px 여유만 남기고 최대한 꽉 채움(하단 빈칸 최소화)
    if (Math.abs(delta) <= 2 || _vfitIter > 16) return;        // 충분히 수렴 or 반복 상한 → 정지
    _vfitIter++;
    _vfit += delta * 0.7;                                      // 감쇠 적용해 목표로 수렴
    if (_vfit < -80) _vfit = -80; if (_vfit > 480) _vfit = 480;
  ...
  function handEdgeMove(e) {
    if (isTouchDevice()) return; // 터치기기: 합성 mousemove 로 엣지 자동스크롤이 돌면 손패가 계속 튕긴다 → 네이티브 스크롤만 사용
  ...
  function handCardEl(id, i, mode) {
    ...
    var effFs = (big ? 10 : 9.5) - (_long ? 1.5 : 0);
  ...
  function miniHandCard(id, i, mode) {
    ...
    var playable = mode === 'play' ? (isP ? G.canCast(HUMAN, id) : G.canDeclare(HUMAN, id)) : false;
    var seld = (mode === 'play' && sel && sel.type === 'hand' && sel.i === i) || (ptr && ptr.i === i);
    var W = 100, H = 134, cl = CLS[card.cls] || CLS.generic;   // 패 높이 축소 → 그만큼 필드(보드)가 커진다(모바일).
