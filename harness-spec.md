...
### 조사 결론
카드 풀 222종은 **전부 고유 art 파일** 보유. 유일한 실제 파일 중복은:
- `Token2` → `art/Token2.png`, `Token21` → `art/Token2.png` (art-map.js:113-114). 둘 다 토큰, 도감·덱빌더 풀에서 **제외**(deny). 인게임 소환 시에만 노출.

### 결정
- `Token21`을 **고유 아트로 분리**. 신규 PNG 생성은 이 세션 범위 밖(자산 파이프라인 없음)이므로:
  - (기본안) `Token21`의 실제 인게임 용도 확인 후, 시각적으로 구분되는 기존 토큰 아트(예: `Token2b.png`)로 **재지정**하거나, 두 토큰이 사실상 동일 개념이면 **Token21 정의를 Token2로 통합**.
- 그 외 "겹쳐 보이는" 것은 **시각적 유사**일 수 있음 → 토큰/내부 아트(`__body`, `__wall`) 스팟체크로 확인. 실제 서로 다른 카드 간 파일 공유는 없음.
> 사용자가 특정 카드쌍을 지목하면 그에 맞춰 조정. 현 데이터 기준 조치는 Token21 분리뿐.
